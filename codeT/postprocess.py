...
class PostProcessor:
    ...
    @staticmethod
    def map_task_id_for_solution1(predict_path):
        result = []
        predictions = Tools.load_jsonl(predict_path)
        for pre in predictions:
            entry_point = get_function_name(pre['problem'])
            pre['entry_point'] = entry_point
            if not pre['solutions']:
                result.append({
                    'task_id': entry_point,
                    'prompt': pre['problem'],
                    'test': '',
    ...
    @staticmethod
    def map_task_id_for_test_case1(predict_path):
        test_cases_by_task = defaultdict(list)
        predictions = Tools.load_jsonl(predict_path)
        for pre in predictions:
            entry_point = get_function_name(pre['problem'])
            for sample in pre['tests']:
                test_cases = PostProcessor.test_case_extract(sample, entry_point)
                test_cases_by_task[entry_point].append(test_cases)
        return test_cases_by_task
